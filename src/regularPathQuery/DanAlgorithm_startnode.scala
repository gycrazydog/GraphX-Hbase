...
case class SrcId(srcid : Long) 
  ...
  def run(workerNum:Int):Unit = {
    ...
    val initialNode = GraphReader.getStartNode(sc,path)
    ...
    val currentNodes = sc.cassandraTable(keyspace, tableName).where("inputnode = true")
                          .union(sc.parallelize(Array(initialNode))
                                .map(v=>Complex(v,null,null))
                                .joinWithCassandraTable(keyspace, tableName)
                                .map(f=>f._2))
                          .coalesce(3)
                          .cache()
     val temp = currentNodes.filter(f=>f.getInt("srcid")==750).collect()
    temp.foreach(println("750 node : ",_))
    println("after currentNodes")
    val startSets = currentNodes
                          .map(v=>v.getInt("srcid"))
                          .distinct()
                          .collect()
                          .toSet
    println("after startsets")
...
//      val nextStates = currentStates.
    ...
    var current = masterStates.filter(p=>p._1.srcId==1L&&initialNode==p._2._1)
